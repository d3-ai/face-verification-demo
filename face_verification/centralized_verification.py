...
def main():
    ...
    optimizer = torch.optim.SGD(net.parameters(), lr=0.005, momentum=params_config["momentum"], weight_decay=params_config["weight_decay"])
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=100, eta_min=0.005)
    scheduler = CosineAnnealingWarmupRestarts(optimizer,first_cycle_steps=80,cycle_mult=1.0,max_lr=params_config["lr"],min_lr=0.005,warmup_steps=20,gamma=1.0)
