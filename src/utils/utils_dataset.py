...
from dataset_app.centralized_dataset import CentralizedCelebaVerification
from dataset_app.federated_dataset import (
    CIFAR10_truncated,
...
def load_centralized_dataset(dataset_name: str, train: bool = True, target: str = None, download: bool = False)->Dataset:
...
def load_federated_dataset(dataset_name: str, id: str, train: bool = True, target: str = None, download: bool = False)->Dataset:
    if dataset_name == "CIFAR10":
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4914,0.4822,0.4465),(0.2023,0.1994,0.2010))])
        dataset = CIFAR10_truncated(root=DATA_ROOT, id=id, train=train, target=target, transform=transform, download=download)
    elif dataset_name == "CelebA":
        assert target is not None
